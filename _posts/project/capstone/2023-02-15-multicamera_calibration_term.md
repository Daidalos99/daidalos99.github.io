...
선형대수학 용어 정리에는 아래 링크를 참고였다.  
https://losskatsu.github.io/linear-algebra/rank-dim/#%EB%B6%80%EB%B6%84%EA%B3%B5%EA%B0%84subspaces
...
- Basis(기저)  
    ...
    즉, 2차원 좌표평면을 예로 든다고 하면 **(1,0),(0,1)**의 경우 여기에 각각 실수를 곱하고 더하면(이 벡터조합을 이용하면), 2차원 좌표평면의 **어떤 벡터든지 나타낼 수 있다**. 따라서 이는 2차원 좌표평면을 나타내는 **기저(basis)**벡터라고 할 수 있다. (x,0),(0,y)의 꼴이라면 모두 기저(basis)가 될 수 있지만, 기저를 구성하는 벡터의 수는 유일하다. 2차원의 경우 좌표평면을 구성하는 기저벡터는 2개이다. **기저는 벡터공간을 생성하는 선형독립인 벡터이다.**  
- Subspace  
