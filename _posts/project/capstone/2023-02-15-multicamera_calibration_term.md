...
- **Span**  
    전체 벡터공간이 5차원이고, 3개의 기저 벡터 집합을 \\(S\\)라고 하고, 집합 \\(S\\)\\)에 속하는 기저 벡터들로 구성되는 3차원 부분 공간을 \\(W\\)라고 했을 때 \\(S는 부분 공간 \\(W\\)를 span한다고 말하고 \\(W = span(S)\\) 라고 표현한다. 위의 예에서는 전체가 5차원 이지만 기저벡터가 3차원까지만 표현 가능하므로, span되는 공간은 \\(W = span(S) = 3\\)차원이라고 할 수 있다.  
- **Rank**  
